...
;( function( $, window, document, undefined ) {
	...
	var nextButton = $j('.product-img-box [role="button"].next');
	var previousButton = $j('.product-img-box [role="button"].previous');

	/**
	 * Update the display of the next/previous buttons
	 *  - if selected gallery image is first, hide previous button
	 *  - if selected gallery image is last, hide next button
	 */
	...
	// trigger a click on the next thumbnail
	nextButton.on('click', function (e) {
		$j('.more-views .selected + li .lightbox').trigger('click');
	});

	// trigger a click on the previous thumbnail
	previousButton.on('click', function (e) {
		$j('.lightbox', $j('.more-views .selected').prev()).trigger('click');
	});

	// whenever a thumbnail is clicked, trigger a button update (once the switch is resolved)
	$(document).on('click', '.more-views li .lightbox', function () { setTimeout(updateButtonDisplay, 10); });
